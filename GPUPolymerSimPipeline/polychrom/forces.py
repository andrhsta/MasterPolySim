...
def Lennard_jones(
    ...
    """
    Force function for the standard Lennard-Jones potential with a cutoff to 
    describe interactions between particles. The potential can be optionally
    shifted to ensure  that both the potential and its first derivative go smoothly to zero
    at the cutoff distance.
    
    Some code inspiration taken from: https://openmm.github.io/openmm-cookbook/latest/notebooks/tutorials/Alchemical_free_energy_calculations.html
    and theoretical information from https://docs.openmm.org/latest/userguide/theory/02_standard_forces.html
    

    The LJ interaction is described by the energy function:
        E(r) = 4 * epsilon * ((sigma/r)**12 - (sigma/r)**6)
    where:
        r is the distance between particles,
        epsilon is the depth of the potential well (sets the strength of the interaction),
        sigma is the distance at which the energy is zero.

        
    Parameters:

    epsilon: float
        the depth of the potential well (sets the strength of the interaction).
    sigma: float
        the distance at which the energy is zero.
    cutoff: float
        cutoff radius in units of sigma.
    shift: bool
        if True, the potential is shifted to ensure that both the potential and its first derivative go smoothly to zero
        at the cutoff distance (U(rcut) = 0 and dU/dr|rcut = 0)
    name: str
        name of the force
    """

    cutoff_dist = cutoff * sim_object.conlen * sigma

    if shift:
        # Calculate the shift value to ensure U(rcut) = 0
        shift_value = 4 * epsilon * ((sigma / cutoff_dist)**12 - (sigma / cutoff_dist)**6)
        energy = (
            "4 * epsilon * ((sigma/r)^12 - (sigma/r)^6) - shift_value;"
            "shift_value = 4 * epsilon * ((sigma/cutoff_dist)^12 - (sigma/cutoff_dist)^6);"
        )
    else:
    ...
    force.addGlobalParameter("epsilon", epsilon * sim_object.kT)
    force.addGlobalParameter("sigma", sigma * sim_object.conlen)
    force.addGlobalParameter("cutoff_dist", cutoff_dist)
